...
$help = array(
	...
	array(
		0 => '我已經完成註冊但是無法登入！',
		1 => '首先，確認您輸入的會員名稱和密碼是否正確。如果是，那麼可能是因為您的帳號尚未啟用。如果您收到了 e-mail，那請就按照其中的步驟完成啟用，如果您沒有收到 e-mail，那麼您註冊的 e-mail 位址可能不正確或者是被當作是廣告信而過濾掉。如果您肯定 e-mail 位址正確，那麼請聯絡管理員。'
	),
	array(
		0 => '我過去已經註冊（登入）過，但是現在卻無法登入？！',
		1 => '很有可能管理員由於某種原因，停用或刪除了您的帳號。'
	),
	array(
		0 => '我為什麼不能註冊？',
	...
	array(
		0 => '顯示的時間不正確！',
		1 => '一般很少出現伺服器時間不準的情況，您看到的時間不準有可能是因為討論區和您處於不同的時區。如果是這種原因，您可以在個人資料中更改時區，例如倫敦、巴黎、紐約、雪梨、...等等。'
	),
	array(
		0 => '我更改了時區但是時間還是顯示錯誤！',
		1 => '如果您確認您已經設定了正確的時區而時間依然錯誤，這很有可能是因為日光節約時間的影響（使用於大不列顛聯合王國或其他的國家）。討論區並未對標準時間和日光節約時間之間的變更做周密的處理，所以在夏季的月份裡時間有可能會和當地時間有一個小時的時間差。'
	),
	array(
		0 => '我如何才能在自己的名字下顯示圖像呢？',
		1 => '在瀏覽文章時，可能會有兩個圖像和會員名稱一塊顯示。第一個是和您的等級相關的圖像，一般以星星、方塊或小圓點的形式，顯示您在這個討論區的地位，或者您已經發表了多少篇的文章。第二個通常是一個更大的圖像，這是會員的頭像，一般是具有獨特的或個人的表徵。是否允許頭像由討論區的管理員決定。如果您不能使用頭像，那麼這可能是因為管理員關閉了頭像功能的結果。您可以詢問他們關閉的原因。'
	),
	array(
		0 => '如何改變我的等級？',
	...
	array(
		...
		1 => '很抱歉！只有註冊會員才能透過討論區發送 e-mail 給其他會員。這主要是為了防止匿名使用者利用此系統發送垃圾郵件。'
	),
	array(
		0 => '--',
		1 => '發表文章相關問題'
	...
	array(
		0 => '我該如何在版面上發表主題？',
		1 => '在版面上發表文章，請點選相應的按鈕。您所擁有的權限列表顯示在版面和文章頁面的下方（比如 <em>您可以發表新主題、您可以參與投票、...等</em> 這樣的列表）。'
	),
	array(
		0 => '我該如何編輯或刪除一篇文章？',
		1 => '你不能編輯或刪除一篇文章，但管理員會在合理情況下刪除文章。合理情況包括：對公眾 會／將會／可能 構成危險／傷害／恐慌，例如<a href="http://evchk.wikia.com/wiki/網民聲稱在港鐵放炸彈事件">此事件</a>。'
	),
	array(
		0 => '我該如何在我的文章後增加簽名？',
	...
	array(
		0 => '我該如何建立一個投票？',
		1 => '您可以很容易地建立一個投票，當您發表或者修改文章的時候，您可以在頁面下方看到一個「建立票選活動」的標籤。您需要為投票輸入一個票選問題和至少兩個票選項目。您可以設定投票的時間限制（0 表示沒有時間限制）。對於投票的選項數目，討論區會有一個限制，這由管理員設定決定。'
	),
	array(
		0 => '為什麼我不能增加更多的投票選項？',
		1 => '投票選項數量的限制由管理員設定。如果您覺得需要增加允許的投票選項數量，請聯絡管理員以提出意見。'
	),
	array(
		0 => '我該如何編輯或刪除一個投票？',
		1 => '投票只能由發起者，版主，或管理員修改。要編輯一個投票，請點選編輯該主題的第一篇文章；投票的相關設定總是在此。如果還沒有人參與投票，會員可以刪除投票或編輯投票選項，但是一旦已經有人參與投票，就只有版主或管理員可以編輯或刪除它。這是為了防止在投票過程中竄改投票選項。'
	),
	array(
		0 => '為什麼我不能訪問某個版面？',
		1 => '一些版面是有訪問限制的。要檢視、閱讀、發表或執行其他的動作，您需要特別的權限。'
	),
	array(
		0 => '為什麼我不能上傳附加檔案？',
		1 => '這個討論區不能上傳附加檔案。'
	...
	array(
		0 => '為什麼我收到了一個警告？',
		1 => '每個管理員對自己的討論區都有一套規則。如果您違反了規則，那麼您也許會收到一個警告。如果您不確知為何會收到一個警告，那麼請聯絡討論區管理員。'
	),
	array(
		0 => '我該如何向版主檢舉一個文章？',
		1 => '請在您想檢舉的文章上點選檢舉的按鈕，而後按照提示的步驟完成檢舉。'
	),
	array(
	...
	array(
		0 => '什麼是 BBCode？',
		1 => 'BBCode 是一種特殊的 HTML 實現方式。BBCode 本身和 HTML 風格相似，每個標籤用方括弧 [ 和 ] 而不是 &lt; 和 &gt; 並且這種方式提供更多的顯示控制。<br />請瀏覽 BBCode 說明以取得詳細資訊。'
	),
	array(
		0 => '我可以使用 HTML 語法嗎？',
	...
	array(
		0 => '表情符號是甚麼？',
		1 => '表情符號，是能夠表示心情的小圖案，它是使用一個簡短的代碼產生，例如，:) 表示快樂的，而 :( 表示悲傷的。所有表情符號的列表可以在發文的頁面看到。<br /><em>請勿過度使用表情符號。</em>'
	),
	array(
		0 => '我能貼圖嗎？',
		1 => '您可以在文章貼圖。您必須使用連結去顯示儲存在<em>公開</em>網站上的圖檔，而不是直接連結到儲存在您的電腦（例如C Drive 貼圖大法）需要授權或受密碼保護的圖檔。<br />要顯示連結的圖檔，請使用 BBCode [img] 標籤。'
	),
	array(
		0 => '全域公告是甚麼？',
	...
	array(
		0 => '我不能發送私人訊息！',
		1 => '管理員停用了私人訊息功能，或者取消了您使用私人訊息權限。如果屬於最後一種情況，請連絡管理員以獲得更多的資訊。'
	),
	array(
		0 => '我老是收到不想要的私人訊息！',
	...
	array(
		...
		1 => '這個討論區的 e-mail 表單特徵包含可以測試與追蹤寄件者的保護資訊，您應該將它轉寄給管理員，其中包含寄件者的詳細資料是非常重要的，管理員將可依此採取適當的行動。'
	),
	array(
		0 => '--',
		1 => '好友 &amp; 黑名單'
	...
	array(
		0 => '我的最愛和訂閱有何不同？',
		1 => '在 phpBB3 中，我的最愛功能類似於您在瀏覽器中的我的最愛，當主題有更新時您不會收到提示，但是您可以在我的最愛中隨時檢視它。而訂閱將會在每次版面或主題內容更新時，透過您指定的方式通知您。'
	),
	array(
		0 => '我要如何訂閱版面或主題？',
	...
	array(
		0 => '誰寫了這個討論區程式？',
		1 => '這個軟體（未經修改的版本）的開發及釋出，著作版權歸於 <a href="http://www.phpbb.com/">phpBB Group</a>。遵循 GNU General Public Licence 版權宣告，可以自由使用和發佈，'
	),
	array(
		0 => '關於這個討論區上的濫用或法律上的相關事務，我該向誰反應？',
		1 => '您可以向任何一位管理員反映。<br/>請注意！phpBB 團隊<strong>沒有權力</strong>和義務來管理使用這個討論區的會員行為。請勿向 phpBB 團隊詢問<strong>沒有直接關係</strong>到 phpBB.com 網站之任何法律問題。如果您給 phpBB 團隊寄送<strong>關於任何第三者</strong>使用此軟體的信件，都將可能獲得簡短的聲明或不予回覆。'
	),
	array(
		0 => '免責聲明',
		1 => '「香港熄燈討論區」以即時上載留言的方式運作，本網站對所有用戶上載的內容(包括但不限於圖片、網頁、文字、影像、音效及程式) 的真實性、完整性及立場等，<b>不會負上任何法律責任</b>。<br />受運作方式限制，管理員不能完全監察所有留言，如用戶發現任何問題，請盡快聯絡我們以便跟進。'
	),
);
